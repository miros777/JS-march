...
=========================
*/
function getNamePageNow(path = window.location.pathname, namePage) {
    let slicePath = path.split('/');
    let checkPage = slicePath.find(el => {
        if (el === namePage) {
            return el;
        }
    });
...
if (getNamePageNow(window.location.pathname, 'index.html') === 'index.html') {
    ...
    let obj = {date: newDate.getDate(), month: newDate.getMonth(), year: newDate.getFullYear(), hours: newDate.getHours(), min: newDate.getMinutes()};
...
if (getNamePageNow(window.location.pathname, 'session.html') === 'session.html') {
    let res = localStorage.getItem('sessions')
