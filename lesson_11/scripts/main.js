...
fetch('https://dummyjson.com/carts')
    ...
    .then(({carts}) => {
        ...
        for (const cartsAllElement of carts) {
            ...
            let data = `<b style="font-size: 20px; background: pink">CART</b> id: ${cartsAllElement.id}<br>
            ...
            let res = '<b style="background: dodgerblue">PRODUCTS CART</b><br><br>';
...
fetch('https://dummyjson.com/recipes')
    ...
    .then(({recipes}) => {
        console.log(recipes);
        let divIngredients = document.getElementById('ingredients');

        for(let rec of recipes) {
            let divOneRec = document.createElement('div');
            divOneRec.setAttribute('class', 'oneRec');

            let oneRecContent = `caloriesPerServing: ${rec.caloriesPerServing}<br>
