...
function User1(id: number, name: string, surname: string, email: string, phone: number) {
    this.id = id;
    this.name = name;
...
let generNumber = function getNewNum(): string {
    ...
    let num: string = Math.round(Math.random() * 10000000).toString();
...
function makeUsersObj(users) {
    for (let i: number = 1; i <= 10; i++) {
        users.push(new User1(i, `Vasya_${i}`, `Lom_${i}`, `${i}_ddd_${i}@gmail.com`, Number(generNumber())));
    }
    return users;
}

let usersNew: any[] = [];
let users2: { id: number, name: string, surname: string, email: string, number: number }[] = makeUsersObj(usersNew);
console.log(users2);
/*
- Взяти масив з  User[] з попереднього завдання, та відфільтрувати , залишивши тільки об'єкти з парними id (filter)
*/
console.log(users2.filter(elem => elem.id % 2 == 0));
/*
...
class Client {
    ...
    constructor(id: number, name: string, surname: string, email: string, phone: string, order: object) {
        this.id = id;
        this.name = name;
        this.surname = surname;
        this.email = email;
...
*/
console.log(clients.sort((a, b) => Object.keys(a.order).length - Object.keys(b.order).length));
...
*/
function Car5(id: number, model: string, producer: string, year: number, speed: number, volume: string) {
    this.id = id;
    this.model = model;
    ...
    this.drive = function (): string {
        return `їдемо зі швидкістю ${this.spead} на годину`;
    }
    this.increaseMaxSpeed = function (newSpeed): number {
        return this.spead += newSpeed;
    }
    this.changeYear = function (newValue): number {
        return this.year = newValue;
    }
    this.addDriver = function (driver): void {
        this.driver = driver;
    }
}

let car1 = new Car5(1, 'Megane', 'Renault', 1999, 200, '2.0');
...
class CarNew {
    ...
    constructor(id: number, model: string, producer: string, year: number, speed: number, volume: string) {
        this.id = id;
        this.model = model;
        this.producer = producer;
        this.year = year;
...
let carNew:CarNew = new CarNew(1, '500-C', 'MB', 1599, 300, '2.0');
...
interface Array<T> {
    foreachCustom: (callback: any) => void;
}

Array.prototype.foreachCustom = (callback): void => {
    for (let i: number = 0; i < this.length; i++) {
        callback(this[i]);
    }
}

let arr555: number[] = [1, 2, 33];
arr555.foreachCustom(el => console.log(el));

interface Array<T>{
    filterCustom: (callback: any) => object;
}
...
let newPopelArr: Object = popelushechki.mapCustom((elem) => {
    return {
        ...elem, age: elem.age * 2
    };
});
