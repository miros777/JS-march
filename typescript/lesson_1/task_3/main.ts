...
let count2: number = 0;
while (count2 < 20) {
    document.write(`<h1>Block ${count2 + 1}. ID: ${count2}</h1>`);
    count2++;
}
...
*/
let arrAny: (string | boolean | number)[] = ['Vasya', 'Petya', 123, 666, true];
...
*/
let arrAny2: (number | string | bigint | any)[] = [2, 17, 13, 6, 22, 31, 45, 66, 100, -18];
let i: number = 0;
let arrLength: number = arrAny2.length;
while (i < arrLength) {
...
let arrAny22: number[] = [2, 17, 13, 6, 22, 31, 45, 66, 100, -18];
...
*/
console.log('---');
let arrFirst: (string | boolean | object | number)[] = [false, true, null, 111, 'ladsda', ['wqewqe', 999], {name: 'Giga'}, 0.9, 777, 'Pink'];
...
*/
console.log('---');
let arrFirstMy: (string | boolean | number)[] = [false, true, false, 111, 'ladsda', 'dsdsd', 51511, 555, 777, 'Pink'];
...
*/
let arrBooks: {name: string, pages: number, author: string[], zhanr: string[]}[] = [
    {
        name: 'Jimi 777',
        pages: 10290,
        author: ['Gary'],
...
*/
let resMaxSizeName: number = 0;
let bookName2: string  = '';
for (let j = 0; j < arrBooks.length; j++) {
...
*/
let resMaxAuthor:string = 'Немає такої книги';
let bookRes: any[] = [];
